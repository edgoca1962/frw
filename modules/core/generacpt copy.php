...
class GeneraCPT
{
   ...
   function add_custom_post_types()
   {
      ...
      foreach ($this->atributos as $type => $data) {
         ...
         $this->set_atributo('singular', $type);
         $this->set_atributo('plural', $data['plural']);
   ...
   {
      $singular = $this->get_atributo('singular');
      $plural = $this->get_atributo('plural');
      $cpt_categoria = $singular . '_categoria';
      // Argumentos para la taxonomía
      $args = [
         'labels' => [
            'name'                       => __("Categorías de $plural", 'WPFRW'),
            'singular_name'              => __("Categoría de $singular", 'WPFRW'),
            'menu_name'                  => __("Categorías de $plural", 'WPFRW'),
            'all_items'                  => __('Todas las Categorías', 'WPFRW'),
            'edit_item'                  => __('Editar Categoría', 'WPFRW'),
            'view_item'                  => __('Ver Categoría', 'WPFRW'),
            'update_item'                => __('Actualizar Categoría', 'WPFRW'),
            'add_new_item'               => __('Añadir Nueva Categoría', 'WPFRW'),
            'new_item_name'              => __('Nombre de la Nueva Categoría', 'WPFRW'),
            'parent_item'                => __('Categoría Padre', 'WPFRW'),
            'parent_item_colon'          => __('Categoría Padre:', 'WPFRW'),
            'search_items'               => __('Buscar Categorías', 'WPFRW'),
            'popular_items'              => __('Categorías Populares', 'WPFRW'),
            'separate_items_with_commas' => __('Separar categorías con comas', 'WPFRW'),
            'add_or_remove_items'        => __('Añadir o eliminar categorías', 'WPFRW'),
            'choose_from_most_used'      => __('Escoger de las más utilizadas', 'WPFRW'),
            'not_found'                  => __('No se encontraron categorías', 'WPFRW'),
         ],
         'hierarchical'          => true, // true para comportarse como categorías
         'show_in_nav_menus'     => true,
         'show_ui'               => true,
         'show_admin_column'     => true,
         'rewrite'               => ['slug' => "$singular-categoria"],
         'show_in_rest'          => true, // Mostrar en la REST API
         'rest_base'             => $cpt_categoria, // Ruta base para la REST API
      ];

      // Registrar la taxonomía
      register_taxonomy($cpt_categoria, [$singular], $args);
   }
}
